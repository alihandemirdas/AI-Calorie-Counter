...
export default function HeightScreen({ navigation, route }) {
  ...
  const handlePress = () => {
    if (height.trim() === "") {
      Alert.alert("Hata", t('height-error'));
    } else {
      navigation.navigate("Weight", { age, gender, height });
    }
  };
    ...
    >
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <View style={styles.innerContainer}>
          {/* 2.Adım */}
          <Text style={styles.label}>{t('height-label')}</Text>
          <TextInput
            placeholder={t('height-place')}
            style={styles.input}
          ...
          >
            <Pressable style={styles.pressableButton} onPress={handlePress}>
              <Text style={styles.buttonText}>{t('height-button')}</Text>
            </Pressable>
          </LinearGradient>
        </View>
      </TouchableWithoutFeedback>
